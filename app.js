...
const STORAGE_KEY = "ebenisteriequest_state_v1";
...
const ACCESS_CODE = "EBEN-2026-BOI8"; // code fourni au centre de formation après achat de la licence
...
const SYNC_KEY = "ebenisteriequest_sync_pending";
...
function deviceId() {
  let id = localStorage.getItem("ebenisteriequest_device_id");
  if (!id) {
    id = "cq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("ebenisteriequest_device_id", id);
  }
  return id;
}
